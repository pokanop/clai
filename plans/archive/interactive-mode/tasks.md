...
<!-- Last Updated: 2026-04-26 (implementation complete) -->
...
**Shipped (2026-04-26):** Phase 1 (tasks 1.1–1.17) and Phase 2 (tasks 2.1–2.4). Phase 3 remains deferred per PRD. See `decisions.md` in this directory for implementation notes.

---

## Phase 1: MVP (default session + warm local + pre-run + tri-state) — 17/17 complete
...
- [x] **1.1 Define interactive execution tri-state and effective-mode resolution** `[P0]` `[L]`
  - **Depends on**: None
  - **Requirements**: FR-13, FR-14, FR-15, FR-19, US-5, NFR-5 (resolution logic), SC-6
  - **Acceptance Criteria**:
    - [x] Rust type represents **dry-run**, **confirm**, and **auto** with stable serde names for TOML.
    - [x] `resolve_effective_interactive_mode` (or equivalent) implements precedence **CLI > env > config > built-in default** and is unit-testable without GGUF.
    - [x] When the new config field is **absent**, legacy `policy.dry_run_default == true` maps to interactive **dry-run** (PRD §6 default). When absent and `dry_run_default == false`, use **confirm** for interactive mode unless CHANGELOG documents a different explicit rule—**auto** must not be the silent default for absent key.
    - [x] When the new key is **present**, it is authoritative for interactive mode regardless of `dry_run_default`.
...
- [x] **1.2 Config version / migration for new interactive fields** `[P0]` `[M]`
  - **Depends on**: Task 1.1
  - **Requirements**: FR-19, PRD §6 Migration
  - **Acceptance Criteria**:
    - [x] If `config_version` bump is required, [`src/migrate.rs`](../../src/migrate.rs) includes an `apply_step` and `clai migrate` remains coherent.
    - [x] Existing installs without the new key get deterministic defaults per Task 1.1.
    - [x] No unbounded breaking change: old configs load or get a clear migration error with a one-line fix hint.
...
- [x] **1.3 Global CLI flags and `--yes` semantics for the default session** `[P0]` `[M]`
  - **Depends on**: Task 1.1
  - **Requirements**: FR-15, FR-18, PRD Open Q1 (defaults), US-5
  - **Acceptance Criteria**:
    - [x] At least one global flag overrides interactive execution mode for the process; names appear in **top-level** `clai --help`.
    - [x] Env override names are listed in `--help` (and README).
    - [x] **`--yes`** on the default session is documented: maps to **auto** (force non-interactive run prompt) **and** preserves existing policy auto-confirm behavior where applicable—exact wording matches implementer choice but matches PRD default proposal.
  - **Notes**: Today [`src/main.rs`](../../src/main.rs) `Cli` has only `config`/`model` globals; mirror patterns from `Ask` flags where sensible.

- [x] **1.4 Bare `clai` default route, TTY gating, and optional `interactive` subcommand** `[P0]` `[L]`
  - **Depends on**: Tasks 1.1, 1.3
  - **Requirements**: FR-1, FR-2, FR-12, US-1, US-8, SC-0, SC-7
  - **Acceptance Criteria**:
    - [x] With **stdin and stdout both TTY**, `clai` with no subcommand and no extra args starts the **same** session runner as optional `clai interactive` (FR-2).
    - [x] If stdin or stdout is **not** a TTY, the process **does not** block on input; prints concise usage or top-level help plus a one-line hint to use `clai ask '…'`; **exit code 2** (PRD FR-12 default recommendation).
    - [x] `clap` layout uses a non-required subcommand pattern (or equivalent) so bare invocation is valid on TTY.
...
- [x] **1.5 TTY message styling with severity levels and `NO_COLOR` support** `[P0]` `[M]`
  - **Depends on**: None
  - **Requirements**: NFR-2, NFR-3, US-7, SC-3, SC-4
  - **Acceptance Criteria**:
    - [x] When stdout is a TTY, informational / success / warning / error paths are visually distinct (labels and/or color).
    - [x] When `NO_COLOR` is set, **no** ANSI color codes are emitted for those messages.
    - [x] Piped/non-TTY output avoids interactive-only framing where documented.
  - **Notes**: PRD allows a small crate (`anstream`, `owo-colors`, etc.) with license review; gate styling on `IsTerminal` like existing code.

- [x] **1.6 Pre-run proposal presentation module** `[P0]` `[L]`
  - **Depends on**: Task 1.5 (for styled output; can stub initially)
  - **Requirements**: FR-4, FR-6, US-2, SC-5
  - **Acceptance Criteria**:
    - [x] Given a parsed [`CommandProposal`](../../src/schema.rs) and policy outcome metadata, render: executable + args (or clear shell line when `needs_shell`), a **what it does** line, **why** from `reason` or an explicit **no rationale provided** message, optional `cwd`, optional confidence, policy blocked / requires-confirmation hints.
    - [x] Presentation runs **before** any `executor` invocation for the turn (FR-6).
    - [x] **Dry-run** interactive mode still runs presentation; only execution is omitted.
  - **Notes**: Keep pure formatting testable without subprocess; avoid duplicating policy decisions inside the formatter.

- [x] **1.7 System prompt: encourage structured `reason` field** `[P0]` `[S]`
  - **Depends on**: None
  - **Requirements**: FR-4
  - **Acceptance Criteria**:
    - [x] `build_system_prompt` (or shared builder) instructs the model to populate `reason` explaining **why** the command was chosen.
    - [x] JSON-only / schema constraints remain unchanged.
  - **Notes**: Current prompt in [`src/main.rs`](../../src/main.rs) `build_system_prompt`; share between `ask` and session.

### Engine and session loop

- [x] **1.8 Session-scoped local inference (single GGUF load per process)** `[P0]` `[L]`
  - **Depends on**: None
  - **Requirements**: NFR-1, US-1, SC-1
  - **Acceptance Criteria**:
    - [x] New API holds `LlamaBackend` + `LlamaModel` (or equivalent) for the session; **`complete_local`-style** work reuses them for each user line.
    - [x] `clai ask` continues to work via a thin path (e.g. one-shot wrapper calling the same low-level completion or existing `complete_local_best_effort`) without behavior regression.
    - [x] `#[cfg(feature = "llama")]` gating matches the rest of the crate; `--no-default-features` builds stay green.
...
- [x] **1.9 Cloud completion path for interactive lines** `[P0]` `[M]`
  - **Depends on**: None (can integrate in 1.10)
  - **Requirements**: PRD Phase 1 cloud parity default, Goals §6, FR-4
  - **Acceptance Criteria**:
    - [x] When cloud is selected for a session line, completion uses [`src/cloud.rs`](../../src/cloud.rs) with the same inputs/contract as `cmd_ask`.
    - [x] Connection reuse is **best-effort** without changing API semantics; document actual behavior in PR notes if pooling is minimal.
  - **Notes**: PRD open question defaults to P0 if cost is small.

- [x] **1.10 Interactive session runner (input loop, welcome, prompt prefix, EOF, built-ins)** `[P0]` `[L]`
  ...
  - **Acceptance Criteria**:
    - [x] Repeated line input: each non-empty line is treated as an NL request consistent with `clai ask` text (document trim/empty-line behavior in README).
    - [x] Session start prints short status: local vs cloud, effective interactive mode, model id/path summary.
    - [x] Distinct input prompt or prefix (FR-17).
    - [x] **EOF** ends session with exit **0** when no unrecoverable error.
    - [x] **`exit` / `quit`** (or equivalent) ends without sending to the model; documented in `--help` and in-session help stub.
    - [x] **Ctrl+C** behavior documented (cancel in-flight; double-press / `exit` to leave—per PRD default).
...
- [x] **1.11 Policy integration, FR-16 ordering, and executor parity** `[P0]` `[L]`
  - **Depends on**: Tasks 1.6, 1.10, 1.1, 1.3
  - **Requirements**: FR-5, FR-7, FR-8, FR-16, US-3, US-4, SC-7, NFR-7
  - **Acceptance Criteria**:
    - [x] **Blocked** proposals: show block explanation; **no** “run this?” offer (FR-5).
    - [x] **Allowed** proposals: pre-run presentation **first**; then policy **sensitive** confirmation if needed (default **no**); then interactive **run it?** only in **confirm** mode; then `executor::run_proposal` when permitted (FR-16).
    - [x] **Dry-run** mode: never calls `executor` for policy-approved proposals; skips execution prompts (FR-7).
    - [x] **Auto** mode: skips interactive **run it?**; still runs sensitive policy confirm when required (FR-16, US-4).
    - [x] **Confirm** mode: run prompt default answer is **no** (decline); declining keeps the session open (US-3).
    - [x] Stream strategy, verbose/capture flags, and exit propagation align with `cmd_ask` for equivalent flags (US-4, FR-8).
...
- [x] **1.12 Non-fatal error handling in the session loop** `[P0]` `[M]`
  - **Depends on**: Task 1.10
  - **Requirements**: NFR-4
  - **Acceptance Criteria**:
    - [x] Parse failures, model errors, and other non-fatal issues print a **clear labeled error** and return to the prompt.
    - [x] Only documented unrecoverable resource failures terminate the process; behavior documented in README.
  - **Notes**: Align with tracing levels; avoid logging full user lines at `info!` unless already conventional.

### Observability and documentation

- [x] **1.13 Extend `doctor` with effective interactive execution mode** `[P0]` `[S]`
  - **Depends on**: Task 1.1
  - **Requirements**: PRD Risk (migration confusion), US-5
  - **Acceptance Criteria**:
    - [x] `clai doctor` output includes the **resolved** interactive mode (after overrides) or states defaults unambiguously.
  - **Notes**: [`cmd_doctor`](../../src/main.rs) already prints config snippets; add one line or section.

- [x] **1.14 README and CHANGELOG: precedence, migration, flags, and operator hints** `[P0]` `[M]`
  - **Depends on**: Tasks 1.1, 1.3, 1.4, 1.11
  - **Requirements**: FR-15, FR-18, SC-6, PRD §6
  - **Acceptance Criteria**:
    - [x] README documents tri-state, env/flag names, config key, and **CLI > env > config > default** order.
    - [x] Which `ask` flags apply to default session vs `clai ask` only is explicit (FR-18).
    - [x] CHANGELOG entry describes config migration and default `clai` behavior change.
...
- [x] **1.15 Unit tests for mode resolution, presentation, FR-16 ordering, built-ins, and `NO_COLOR`** `[P0]` `[L]`
  - **Depends on**: Tasks 1.1, 1.5, 1.6, 1.11 (ordering helpers)
  - **Requirements**: PRD §7 Unit tests, NFR-5, QG-1
  - **Acceptance Criteria**:
    - [x] Tests run under `cargo test --no-default-features --locked` without loading GGUF.
    - [x] Coverage includes: precedence tables, empty `reason` copy, blocked vs allowed presentation branches, prompt-order pure functions, `exit`/`quit` line classification, `NO_COLOR` guard.
  - **Notes**: Follow `#[cfg(test)]` / `tests/` layout per repo; see [`src/schema.rs`](../../src/schema.rs) tests as pattern.

- [x] **1.16 Integration test: non-TTY `clai` with no args does not block** `[P0]` `[M]`
  - **Depends on**: Task 1.4
  - **Requirements**: FR-12, PRD Risk (CI hang), QG-1
  - **Acceptance Criteria**:
    - [x] Automated test (or `tests/` harness) invokes the binary with stdin/stdout **not** a TTY and **no** subcommand; process exits quickly with **code 2** (or documented alternative if maintainers change FR-12 decision—update PRD + task).
    - [x] No deadlock waiting for input.
  - **Notes**: Reuse patterns from [`tests/`](../../tests/) (e.g. subprocess with piped stdio).

- [x] **1.17 Phase 1 verification: quality gates and manual TTY smoke** `[P0]` `[M]`
  - **Depends on**: All prior Phase 1 tasks
  - **Requirements**: PRD §7, QG-1–QG-5, SC-1–SC-3 (manual where needed)
  - **Acceptance Criteria**:
    - [x] `cargo test --no-default-features --locked` passes (QG-1, matches [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml)).
    - [x] `cargo clippy --no-default-features --locked -- -D warnings` passes (QG-2).
    - [x] `cargo build --locked` passes with default features / `llama` on a reference machine (QG-3).
    - [x] `cargo fmt --check` passes (QG-4; add to CI in same or adjacent PR if missing per PRD Open Q7).
    - [x] Manual checklist: macOS and Linux TTY—second prompt does not repeat full cold-load feel (SC-1 subjective); severity colors and EOF/`exit` (SC-2, SC-3).
    - [x] Code review sign-off for engine/session boundary and any new dependency (QG-5).
  - **Notes**: CI today runs test+clippy on Ubuntu+macOS without `fmt`; align with maintainers on fmt job.

---

## Phase 2: Polish and parity — 4/4 complete
...
- [x] **2.1 In-session `help` and keybinding / escape documentation** `[P1]` `[M]`
  - **Depends on**: Task 1.10 (session runner)
  - **Requirements**: PRD Phase 2, US-6
  - **Acceptance Criteria**:
    - [x] Built-in `help` lists built-ins, execution modes, and how to exit.
    - [x] Documented in top-level `--help` / README cross-links.
  - **Notes**: Phase 1 may ship a minimal help string; this task expands it.

- [x] **2.2 Optional in-session `reload` for model file changes** `[P2]` `[L]`
  - **Depends on**: Task 1.8
  - **Requirements**: PRD Phase 2, NFR-1 exception path
  - **Acceptance Criteria**:
    - [x] Documented command reloads GGUF/backend; errors are handled without killing the session when possible.
    - [x] Without `reload`, NFR-1 “no repeated full load” still holds for normal lines.
  - **Notes**: Only if maintainers want file-watch complexity—otherwise defer.

### `ask` parity

- [x] **2.3 `clai ask` TTY: optional shared pre-run presentation (verbose or flag)** `[P1]` `[L]`
  - **Depends on**: Task 1.6
  - **Requirements**: PRD Phase 2, FR-18 (clarify flag surface)
  - **Acceptance Criteria**:
    - [x] One-shot `ask` can show the same structured pre-run block as interactive when opted in (e.g. `--verbose` and/or new flag), without breaking script/non-TTY defaults.
    - [x] `--help` documents the behavior.
  - **Notes**: Align with existing `print_only` / `verbose` semantics in [`cmd_ask`](../../src/main.rs).

- [x] **2.4 Phase 2 verification: quality gates** `[P1]` `[M]`
  - **Depends on**: Tasks 2.1–2.3 completed in scope
  - **Requirements**: PRD §7, QG-1–QG-5
  - **Acceptance Criteria**:
    - [x] `cargo test --no-default-features --locked` passes
    - [x] `cargo clippy --no-default-features --locked -- -D warnings` passes
    - [x] `cargo build --locked` passes
    - [x] `cargo fmt --check` passes
    - [x] Manual spot-check of new Phase 2 UX on TTY
